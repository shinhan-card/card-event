...
document.addEventListener("DOMContentLoaded", () => {
  function initInquiryPopup() {
    const modal = document.getElementById("contact-inquiry");
    const openers = document.querySelectorAll("[data-inquiry-trigger]");
    const closers = document.querySelectorAll("[data-inquiry-close]");
    ...
    if (!isDesktop()) {
      ScrollTrigger.create({
        ...
        onEnter: () => {
          animateCountup(big);
          items.forEach((item) => {
            if (item.dataset.countup) {
              animateCountup(item);
            }
          });
          gsap.set(dividers, { height: 1, width: 48, opacity: 1 });
        }
      });
      return;
    }
